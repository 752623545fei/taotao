...
<form id="orderForm" class="hide" action="/order/create.html" method="post">
		...
		<input type="hidden" name="buyerNick" id="buyerNick" value=""/>
...
<div class="step-tit">
	...
	<div class="extra-r">
		...
		<a onclick="alertAddAddressDiag()" class="ftx-05" href="javascript:;">新增收货地址</a>
	</div>
</div>
<div class="step-cont">
	<div class="consignee-list" id="consignee-list1">
		<a href="#none" id="prev" onclick="prevArrow()" class="prev arrow-btns"></a>
		<a href="#none" id="next" onclick="nextArrow()" class="next arrow-btns"></a>
		<script type="text/javascript">
			function nextArrow(){
				var position = $(".ui-switchable-panel-main").css("left");
				var count = $(".ui-switchable-panel").length;
				var page = Math.ceil(count/4);
				position = position.substring(0,position.length-2);
				position = Number(position) - Number(920);
				var temp = -(page*920);
				if (position <= temp){
					return;
				}
				$(".ui-switchable-panel-main").css("left", position+"px");
			}
			function prevArrow(){
				var position = $(".ui-switchable-panel-main").css("left");
			...
			function selectAddress(obj) {
				$(".consignee-item").attr("class", "consignee-item");
				$(obj).children("div").attr("class", "consignee-item item-selected");
				setShippingCode();
			}

			function setShippingCode() {
				var node = $(".item-selected");
				var id = node.attr("consigneeid");
				$("#shippingCode").attr("value", id);
			}
		</script>
		<div id="consignee1" class="list-cont ui-switchable-body">
            <div id="consignee-ret"></div>
   			<ul class="ui-switchable-panel-main" id="consignee-list" style="position: relative ; left: 0px">
				<!---->
				<c:forEach items="${address}" var="address">
					<li class="ui-switchable-panel" onclick="selectAddress(this)" id="consignee_index_${address.addressId}"
						selected="selected" style="cursor: pointer;">

						<div class="consignee-item " default="${address.easyBuy}"
							 consigneeid="${address.addressId}" id="consignee_index_div_${address.addressId}">
							...
							<div class="user-name">
								<div class="fl">
									<strong limit="4">${address.consigneeName}</strong>&nbsp;&nbsp;收
								</div>
								<div class="fr">${address.mobile}</div>
								<div class="clr"></div>
							</div>
							<div class="mt10" limit="15">${address.provinceId}&nbsp; ${address.cityId}&nbsp; ${address.countyId}</div>
							<div class="adr-m" limit="30">${address.consigneeAddress}</div>
							<div class="op-btns ar">
								<a href="#none"
									class="ftx-05 mr10 setdefault-consignee hide"
									fid="${address.addressId}">设为默认地址</a> <a href="#none"
									class="ftx-05 mr10 edit-consignee" fid="137617472">编辑</a>
								<a href="#none" class="ftx-05 del-consignee hide"
									fid="${address.addressId}">删除</a>
							</div>
						</div>
